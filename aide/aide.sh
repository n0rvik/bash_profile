...
# FILE: aide
# INSTALL: yum install aide
# CONFIG:
# sed -i -e '/gzip_dbout/ s/yes/no/; s/aide.db.gz/aide.db/; s/aide.db.new.gz/aide.db.new/' /etc/aide.conf
# sed -i -e '/define LOGDIR/ s/log/lib/' /etc/aide.conf
# sed -i -e '/DBDIR}\/aide.db.new/ a\database_new=file:@@{DBDIR}/aide.db.new'  /etc/aide.conf
# sed -i -e '$ a\\n#Mail\n!/mail\n!/var/spool/mail\n!/mnt/datamail\n' /etc/aide.conf
# sed -i -e '$ a\\n#Log\n!/var/log\n' /etc/aide.conf
#
# OR
